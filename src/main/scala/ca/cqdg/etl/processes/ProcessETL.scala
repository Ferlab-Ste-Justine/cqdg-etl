...
import ca.cqdg.etl.models.{Metadata, NamedDataFrame}
import ca.cqdg.etl.processes.ProcessETLUtils.columns.notNullCol
import ca.cqdg.etl.processes.ProcessETLUtils.loadAll
import ca.cqdg.etl.processes.indexes.{DataAccess, DonorIndex, FileIndex, StudyIndex}
...
class ProcessETL(keycloakClient: IKeycloak)(implicit spark: SparkSession, conf: Configuration) {
  ...
  def transform(data: Map[String, DataFrame]): (DataFrame, DataFrame, DataFrame) = {
   ...
   log.info("Computing Donors ...")
    ...
    if (keycloakClient.isEnabled()) {
      ...
      val future = keycloakClient.createResources(allFilesInternalIDs)
