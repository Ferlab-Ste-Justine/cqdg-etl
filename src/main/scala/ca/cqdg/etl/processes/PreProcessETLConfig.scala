...
class PreProcessETLConfig(input: String, output: String) {
  ...
  val preProcessETLConfig: Configuration = Configuration(
    ...
    sources = List(
      ...
      // data to index
      DatasetConf("studyversionmetadata-with-ids", "output", "/study_version_metadata", PARQUET, OverWrite),
      ...
      DatasetConf("familyhistory-with-ids", "output", "/family-history", PARQUET, OverWrite),
      DatasetConf("family-with-ids", "output", "/family", PARQUET, OverWrite),
      DatasetConf("file-with-ids", "output", "/file", PARQUET, OverWrite),
      DatasetConf("followup-with-ids", "output", "/follow-up", PARQUET, OverWrite),
      DatasetConf("phenotype-with-ids", "output", "/phenotype", PARQUET, OverWrite),
      DatasetConf("sampleregistration-with-ids", "output", "/sample_registration", PARQUET, OverWrite),
